...
const MessageStatus = ({ status }: { status: 'sending' | 'sent' | 'delivered' | 'read' }) => {
  const getStatusIcon = () => {
    switch (status) {
      case 'sending':
        return <ActivityIndicator size="small" color={COLORS.loading} />;
      case 'sent':
        return <MaterialCommunityIcons name="check" size={16} color={COLORS.sent} />;
      case 'delivered':
        return <MaterialCommunityIcons name="check-all" size={16} color={COLORS.sent} />;
      case 'read':
        return <MaterialCommunityIcons name="check-all" size={16} color={COLORS.success} />;
    }
  };
...
const NoMessage = memo(({ messages }: { messages: Message[]}) => (
  <View>
      ...
      <View style={styles.noMessageContainer}>
        <MessagesSquare style={styles.noMessagesIcon} size={50} color={COLORS.lightText} />
      ...
      >
        ...
        >
          ...
          <View style={styles.messageFooter}>
            ...
            {isMyMessage && <MessageStatus status={item.deliveryStatus as 'sent'} />}
          </View>
        </View>
      </Animated.View>
    );
